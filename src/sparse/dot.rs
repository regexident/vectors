...
impl<T, S> Dot for SparseVector<T, S>
where
...
impl<T, S, S2> Dot<DenseVector<T, S2>> for SparseVector<T, S>
where
    T: Copy + Add<T, Output = T> + Mul<T, Output = T> + Zero,
    S: Storage<(usize, T)>,
    S2: Storage<T>,
...
mod test {
    ...
    fn dot_dense() {
        ...
        let expected: f64 = 0.2 * 0.0 + 0.5 * 0.1 + 1.0 * 0.2 + 0.0 * 0.3 + 2.0 * 0.0 + 4.0 * 0.4 + 0.0 * 0.5;
